...
interface DungeonPartyDao {
    ...
    @Query("SELECT * FROM dungeon_party WHERE account_id = :accountId AND dungeon_type IN (:dungeonTypes) AND element_type IN (:elementTypes) AND (title LIKE :title OR monster_name LIKE :title)")
